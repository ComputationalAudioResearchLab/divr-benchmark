...
class Analyser:
    ...
    def run(self, output_file: str, data_file_root: str):
        ...
        for data_file in tqdm(data_files, "processing data"):
            (
                ...
                sampling_rate,
                ssl_type,
                (num_layers, latent_dim, lr, epochs),
            ...
            if epochs not in this_results:
                this_results[epochs] = {}
            this_results = this_results[epochs]
            if lr not in this_results:
                this_results[lr] = {}
            this_results = this_results[lr]
            if num_layers not in this_results:
                this_results[num_layers] = {}
            this_results = this_results[num_layers]
            if latent_dim not in this_results:
                this_results[latent_dim] = {}
            this_results = this_results[latent_dim]
    ...
    def get_file_tags(self, data_file: Path):
        ...
        epochs, lr, num_layers, latent_dim = svm_config.split("_")
        return (svd_type, sampling_rate, ssl_type, (num_layers, latent_dim, lr, epochs))
