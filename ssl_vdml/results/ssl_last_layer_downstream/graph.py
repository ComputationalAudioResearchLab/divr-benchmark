...
class Grapher:
    ...
    def run(self, log_path: str, sample_rate):
        ...
        sns.boxenplot(
            full_df[full_df["exp_type"] == "nn_latents"],
            x="model_type",
            y="val_acc",
            ax=ax,
        )
        ax.set_xlabel(None)
        ax.set_ylabel(None)
        ax.set_title("Validation Accuracy / Model Type")
        fig.suptitle("SSL Last Layer Feature Evaluation")
        plt.savefig("ssl_last_layer_downstream.png")
