...
class Grapher:
    ...
    def run(self, log_path: str):
        ...
        fig, ax = plt.subplots(
            1, 3, figsize=(7, 4), constrained_layout=True, sharey="row"
        )
        self.my_plot(full_df, exp_type="nn_latents_full", ax=ax[0])
        self.my_plot(full_df, exp_type="nn_latents", ax=ax[1])
        self.my_plot(full_df, exp_type="baseline_latents", ax=ax[2])
        fig.suptitle("Validation Accuracy / Sampling Rate")
        plt.savefig("sample_rate.png")

    def my_plot(self, df, exp_type, ax):
        sns.boxenplot(
            df[df["exp_type"] == exp_type],
            x="sample_rate",
            y="val_acc",
            ax=ax,
        )
