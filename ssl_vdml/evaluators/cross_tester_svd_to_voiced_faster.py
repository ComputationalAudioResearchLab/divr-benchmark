...
class CrossTester:
    ...
    def run(self, best_logs: List[str]):
        for log_file in tqdm(best_logs, "on model", position=1):
            ...
            dataset_conf_tag = (
                f"/home/storage/data/{exp_type}/{sample_rate}/features/voiced"
            )
            if dataset_conf_tag not in self.dataset_confs:
                self.dataset_confs[dataset_conf_tag] = self.load_dataset(
                    files=[
                        f"{dataset_conf_tag}_train.json",
                        f"{dataset_conf_tag}_val.json",
                    ]
                )
...
if __name__ == "__main__":
    ...
    tester = CrossTester(
        ...
        diagnosis_level=0,
        output_root="/home/storage/data/cross_tester/svd_to_voiced_faster",
        device=torch.device("cuda"),
    )
    df = pd.read_csv(f"{curdir}/crunched_accuracies.csv")

    def my_filter(logfile):
        if "svd_a." in logfile or "svd_aiu." in logfile:
            if "data2vec" in logfile or "unispeech" in logfile:
                if "nn_latents_full" in logfile:
                    if "16000" in logfile:
                        return True
        return False

    best_logs = [log_file for log_file in df["log_file"] if my_filter(log_file)]
