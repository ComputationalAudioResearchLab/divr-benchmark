        ...
        for log_file in tqdm(best_logs, "on model", position=1):
            ...
            if dataset_conf_tag not in self.dataset_confs:
                self.dataset_confs[dataset_conf_tag] = self.load_dataset(
                    files=[
                        f"{dataset_conf_tag}_a_n_val.json",
                    ]
                )
...
if __name__ == "__main__":
    ...
    tester = CrossTester(
        ...
        diagnosis_level=0,
        output_root="/home/storage/data/cross_tester/voiced_to_svd_a",
        device=torch.device("cuda"),
    )
    df = pd.read_csv(f"{curdir}/crunched_accuracies.csv")
    best_logs = [
        log_file
        for log_file in df["log_file"]
        if ("voiced" in log_file and "rbf" in log_file)
    ]
    print(len(best_logs))
    exit()
    tester.run(best_logs=best_logs[1148:][59:])
