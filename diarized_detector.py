...
class DiarizedDetectorTrainer:
    ...
    def run(
        ...
        self.model = DiarizedDetector(classes=len(self.classMap), device=self.device)
        ...
        for epoch in pbar:
            ...
            if epoch in fig_epochs:
                self.add_confusion_image("confusion_val", valY, epoch, val_predY)
                self.add_confusion_image("confusion_train", trainY, epoch, train_predY)
            pbar.set_postfix(details)

    def add_confusion_image(self, key, valY, epoch, val_predY):
        confusion = confusion_matrix(valY.cpu(), val_predY.cpu())
    ...
    def train(self, trainX, trainY):
        ...
        loss.backward()
        self.optimizer.step()
        return loss.item(), predY.softmax(dim=1).argmax(dim=1).detach()
    ...
    def val(self, valX, valY):
        ...
        return loss.item(), predY.softmax(dim=1).argmax(dim=1)
...
if __name__ == "__main__":
    measure = DiarizedDetectorTrainer(
        diagnosis_level=0,
        output_folder=Path("/home/workspace/output_diarized_detector"),
