...
DataPoint = Union[
    ...
    Tuple[InputTensors, LabelTensor, LabelTensor],
]
...
class BaseDataLoader:
    ...
    def __init__(
        ...
        all_ids = [p.id for p in (task.train + task.val + task.test)]
        all_ids += [f"{extra_db.name}_{k}" for k in range(extra_db.total_data)]
        self._all_ids = {k: v for v, k in enumerate(all_ids)}
    ...
    def get_injection_data(self, current_batch_size: int):
        ...
        if num_extra_audios < 1:
            return 0, [], []
        extra_audios, keys = self.__extra_db.get_audios(num_audios=num_extra_audios)
        num_extra_audios = len(keys)
        extra_labels = [
            [self.__task.normal_index(level) for level in self.__diag_levels]
            for _ in range(num_extra_audios)
        ]
        extra_ids = [f"{self.__extra_db.name}_{k}" for k in keys]
        return extra_ids, extra_audios, extra_labels
